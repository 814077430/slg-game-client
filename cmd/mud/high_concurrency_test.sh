...
# 启动客户端（分批启动避免系统负载过高）
echo "=== 启动 $CLIENT_COUNT 个客户端（高并发随机消息测试）==="
for i in $(seq 1 $CLIENT_COUNT); do
    ...
    (
        ...
        echo "/quit"
    ) | timeout $((TEST_DURATION + 10)) $MUD_CLIENT > /tmp/stress_client_$i.log 2>&1 &
...
for i in 100 500 1000 2000 3000; do
    if [ -f "/tmp/stress_client_$i.log" ]; then
        echo "--- 客户端 $i ---"
        head -30 /tmp/stress_client_$i.log 2>/dev/null | grep -E "(连接 | 登录|>|status|look|who)" | head -10
    fi
done
